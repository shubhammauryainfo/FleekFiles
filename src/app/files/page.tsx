...
import { FaTimes, FaSearch, FaFilter, FaDownload, FaTrash, FaHdd, FaCopy, FaCheck } from "react-icons/fa";
import { FaShare, FaQrcode } from "react-icons/fa";
...
export default function FilesPage() {
  ...
  const siteurl = "https://fleek-files.vercel.app";  // Calculate storage usage
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-200/60 p-6 mb-8">
        ...
        <div className="relative">
          <div className="w-full bg-slate-200 rounded-full h-4 overflow-hidden shadow-inner">
            <div
              className={`h-full rounded-full transition-all duration-1000 ease-out ${storagePercentage >= 90
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 bg-slate-100 rounded-xl p-1">
              <button
                ...
                className={`px-3 py-2 rounded-lg transition-all duration-300 ${viewMode === 'grid'
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg transition-all duration-300 ${viewMode === 'list'
                  ? 'bg-white shadow-sm text-blue-600'
                  : 'text-slate-600 hover:text-slate-800'
                  }`}
              >
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-2 px-4 py-3 
                                         bg-purple-50 text-purple-700 rounded-xl hover:bg-purple-100 
                                           transition-all duration-300 font-medium group-hover:bg-purple-600 hover:text-purple-700 group-hover:text-white"
                          >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            ...
                            <button
                              onClick={() => handleShareFile(file)}
                              className="flex items-center gap-2 px-6 py-3 
    bg-purple-50 text-purple-700 rounded-xl hover:bg-purple-600 hover:text-white
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div className="relative bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md transform transition-all duration-300 scale-100">
      ...
      <div className="text-center">
        ...
        <h3 className="text-2xl font-bold text-slate-800 mb-2">Share File</h3>
        <p className="text-slate-600 mb-2">
          Scan QR code to download
        </p>
        <p className="text-sm text-slate-500 mb-8 truncate">
          {files.find(f => f._id === showShareModal)?.filename}
        </p>
        
        {/* QR Code Container */}
        <div className="bg-white border-4 border-slate-200 rounded-2xl p-6 mb-6 mx-auto w-fit">
          <div 
            id="qrcode"
            className="w-48 h-48 bg-slate-100 rounded-xl flex items-center justify-center"
          >
            {/* QR Code will be generated here */}
            <img 
              src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(`${siteurl}/share/${showShareModal}`)}`}
              alt="QR Code"
        ...
        <p className="text-sm text-slate-600 mb-6">
          <strong>Scan and download your file</strong>
        </p>
        
        <button
          onClick={() => setShowShareModal(null)}
          className="w-full px-6 py-3 bg-slate-100 text-slate-700 rounded-xl 
          hover:bg-slate-200 transition-all duration-300 font-medium"
        >
          Close
        </button>
